...
export GRADLE_HOME=/opt/gradle/6.4.1
export TOOL_VERSION=3.8.8
export PROJECT_VERSION=2.8.9
...
if [ "rpm" = "rpm" ]; then
    microdnf --setopt=tsflags=nodocs install -y java-11-openjdk-jmods
else
    export PATH="/root/project/packages:${PATH}"

    wget --no-verbose --output-document=/root/project/packages/package-0 {URL} \
    && echo "$ /root/project/packages/package-0" | sha256sum --check -

    if [ "executable" = "rpm" ]; then
        chmod +x /root/project/packages/package-0
    elif [  "tar" = "rpm"  ]; then
        mkdir /root/project/packages/package-0-extracted
        tar -xvf /root/project/packages/package-0 --directory /root/project/packages/package-0-extracted
        export PATH="/root/project/packages/package-0-extracted/:${PATH}"
    else
        echo "Unknown Type rpm"
        exit 1
    fi
fi
...
if [ -n "" ]; then
  echo "Setting version to 2.8.9 to match enforced version"
  mvn -B -e -s "/root/software/settings/settings.xml" -t "/root/software/settings/toolchains.xml" org.codehaus.mojo:versions-maven-plugin:2.8.1:set -DnewVersion="2.8.9" | tee /root/project/logs/enforce-version.log
fi
