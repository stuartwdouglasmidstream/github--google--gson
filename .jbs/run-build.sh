...
if [ -n "" ]; then
  echo "Creating tag 2.8.9 to match enforced version"
  git tag -m 2.8.9 -a 2.8.9 || true
else
  echo "Enforce version not set, recreating original tag gson-parent-2.8.9"
  git tag -m gson-parent-2.8.9 -a gson-parent-2.8.9 || true
fi
